                        ...
                        <div class="account-box is-form">
                            <div class="form-head">
                                <div class="form-head-inner">
                                    <div class="left">
                                        <h3>{{$section_title}}</h3>
                                        <p><i class="lnil lnil-warning"></i> Cette section est réservée pour la modification d'un type de congé.</p>
                                    </div>
                                    <div class="right">
                                        <div class="buttons">
                            ...
                            <div class="form-body">
                                <form action="{{route('typeconges.update')}}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" value="{{$typeconge->id}}" name="id">
                                    <div class="columns is-multiline">
                                        <div class="column is-8">
                                            <label for="identifiant">Libéllé <span class="requis">*</span></label>
                                            <div class="field">
                                                <div class="control">
                                                    <input type="text" id="identifiant" class="input" name="libelle" placeholder="Libéllé" value="{{old('libelle', $typeconge->libelle)}}">
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="field">
                                                <div class="control">
                                                    <select class=" form-control p-2" name="status" style="width:100%; heigth:38px !important; border: 1px solid #ccc !important;">
                                                        <option value="1" {{old('status', $typeconge->status) == "1" ? 'selected' : ''}}>Actif</option>
                                                        <option value="0" {{old('status', $typeconge->status) == "0" ? 'selected' : ''}}>Inactif</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
